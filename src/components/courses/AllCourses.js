...
import leaveCourse from './functions/leaveTheCourse/leaveCourse';
        ...
        <div>
            {/* <div> */}
            {
                allCourses.map(course => {
                    return (
                        <div key={course.id} className={'mt-4 shadow-lg border-gray-200 border-2 ml-4 mr-4 rounded-xl relative overflow-y-scroll'}>
                            ...
                            <div>
                                {
                                    enrolledCoursesID.current.indexOf(course.id) === -1 ? <button className='border border-gray-200 mb-2 pl-2 pr-2 font-Mulish text-sm absolute rounded-lg right-6 bottom-0'
                                        onClick={() => {
                                            enrollTheCourse(course.id, user.id);
                                            enroll(course.id)
                                        }}>Enroll Now</button> :
                                        <button className='border border-red-600 mb-2 pl-2 pr-2 font-Mulish text-sm rounded-lg absolute right-6 bottom-0'
                                            onClick={() => {
